    ...
    for fit_intercept in [True, False]:
        for normalize in [True, False]:
            for copy_X in [True, False]:
                for fit_path in [True, False]:
                    for positive in [True, False]:
                        for verbose in [True, False]:
                    
                            clf = ls(precompute=precompute, fit_intercept = fit_intercept, normalize = normalize, copy_X = copy_X, 
                                fit_path = fit_path, positive = positive, verbose = verbose)
                            clf1 = ls(precompute=precompute, fit_intercept = fit_intercept, normalize = normalize, copy_X = copy_X, 
                                fit_path = fit_path, positive = positive, verbose = verbose)
